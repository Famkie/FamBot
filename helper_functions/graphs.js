...
async function stock(stock_acronym, property, interval) {
  let data = await General_functions.http_request(
    "https://tornsy.com/api/" + stock_acronym + "?interval=" + interval
  );
  ...
  try {
    ...
    const property_capitalized =
      property.charAt(0).toUpperCase() + property.slice(1);
    const title =
      property_capitalized.replace("_", " ") +
      " of " +
      stock_acronym.toUpperCase() +
      " with interval " +
      interval;

    let graph = await Graphs_builders.timeline_graph(
      formatted_data,
      title,
      "",
      property.replace("_", " ")
