...
import { Discord_functions } from "./discord.js";
...
async function get_data_from_api(url, user_id = false, private_data = false) {
  if (user_id !== false) {
    const key = await get_users_key(user_id.toString());
    if (key !== "") {
...
async function set_users_key(user_id, guild_id, key = "") {
  ...
  if (key !== "") {
    ...
    if (info.error !== undefined) {
      ...
      let result = await Discord_functions.rename_user(
        user_id,
        guild_id,
        info.name + " [" + info.player_id + "]"
      );
      if (result.error !== undefined) {
  ...
  let operation1 = { replaceOne: { filter, replacement } };

  data.players[user_id] = user;
  await Database.setData(data, { players: [operation1] });
...
async function share_users_key(user_id, share = false) {
  let data = Database.getData();
  let this_player = await General_functions.get_user(user_id);
  ...
  let operation1 = { updateOne: { filter, update } };
  ...
  if (share === true) {
    ...
    for (let i = 0; i < data.general.shared_apis.apis.length; i++) {
